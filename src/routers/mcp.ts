...
router.post('/generate', async (ctx: ParameterizedContext) => {
  // 使用类型断言来处理 body 属性
  const { userPrompt, llmConfig } = (ctx.request as any).body as GenerateRequest;
...
router.post('/test-model', async (ctx: ParameterizedContext) => {
  ...
  try {
    ...
    const { component, reason } = await generateComponent(testPrompt, llmConfig as Partial<LLMConfig>);
